...
import { Eraser as StaticEraser } from "lucide-react";
...
export interface EraserIconProps extends HTMLAttributes<HTMLDivElement> {
  size?: number | string;
  ...
  ({ onMouseEnter, onMouseLeave, className, size, strokeWidth, color, style, ...props }, ref) => {
    ...
    return (
      <motion.div ref={divRef}
        ...
        {...divProps}
      >
        <StaticEraser size="100%" strokeWidth={strokeWidth} color={color} className="w-full h-full" />
      </motion.div>
    );
  }
);
