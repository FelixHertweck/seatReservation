...
import { MoreVertical as StaticMoreVertical } from "lucide-react";
...
export interface MoreVerticalIconProps extends HTMLAttributes<HTMLDivElement> {
  size?: number | string;
  ...
  ({ onMouseEnter, onMouseLeave, className, size, strokeWidth, color, style, ...props }, ref) => {
    ...
    return (
      <motion.div ref={divRef}
        ...
        {...divProps}
      >
        <StaticMoreVertical size="100%" strokeWidth={strokeWidth} color={color} className="w-full h-full" />
      </motion.div>
    );
  }
);
