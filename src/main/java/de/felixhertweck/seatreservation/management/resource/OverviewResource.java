...
import de.felixhertweck.seatreservation.management.service.OverviewService;
...
public class OverviewResource {
     ...
     *
     * @return ManagementOverviewDTO containing aggregated dashboard data
     */
    @GET
    @APIResponse(
            responseCode = "200",
            description = "OK",
            content = @Content(schema = @Schema(implementation = ManagementOverviewDTO.class)))
    @APIResponse(responseCode = "401", description = "Unauthorized")
    @APIResponse(
            responseCode = "403",
            description = "Forbidden: Only MANAGER or ADMIN roles can access this resource")
    public ManagementOverviewDTO getOverview() {
        LOG.debug("Received GET request to /api/manager/overview");
        AuthenticatedUser currentUser = userSecurityContext.getAuthenticatedUser();
        return overviewService.getOverview(currentUser);
    }
}
