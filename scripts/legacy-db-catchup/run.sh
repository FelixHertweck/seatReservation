...
# --clean --if-exists makes the dump drop existing objects first, so it can be restored
# on top of the (modified) database.
echo "==> Backup -> $BACKUP"
docker compose exec -T db sh -c 'pg_dump -U "$POSTGRES_USER" -d "$POSTGRES_DB" --clean --if-exists' > "$BACKUP"
...
echo "==> Done. To restore the backup if needed (replaces the current database contents):"
printf '    docker compose exec -T db sh -c %q < %q\n' \
    'psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1 --single-transaction' "$BACKUP"
